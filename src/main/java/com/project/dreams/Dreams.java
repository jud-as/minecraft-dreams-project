...
public class Dreams {
    ...
    private void commonSetup(FMLCommonSetupEvent event) {
        ...
        if (Settings.LOG_DIRT_BLOCK.getAsBoolean()) {
            LOGGER.info("DIRT BLOCK >> {}", BuiltInRegistries.BLOCK.getKey(Blocks.DIRT));
        }

        LOGGER.info("{}{}", Settings.MAGIC_NUMBER_INTRODUCTION.get(), Settings.MAGIC_NUMBER.getAsInt());

        Settings.ITEM_STRINGS.get().forEach((item) -> LOGGER.info("ITEM >> {}", item));
    }
    ...
    private void addCreative(BuildCreativeModeTabContentsEvent event) {
        ...
        if (event.getTabKey() == CreativeModeTabs.FOOD_AND_DRINKS) {
            event.accept(ItemRegisterService.WILL);
        }
